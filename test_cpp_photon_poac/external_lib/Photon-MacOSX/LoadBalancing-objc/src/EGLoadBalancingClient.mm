...
- (bool) opCreateRoom:(NSString*)gameID :(EGRoomOptions*)options
{
	...
	[roomProps setObject:[options.PropsListedInLobby.Type isEqual:NSStringFromClass([NSString class])]?[[options.PropsListedInLobby copyDeep] autorelease]:[EGArray arrayWithType:NSStringFromClass([NSString class])] forKey:[NSValue value:&Properties::Room::PROPS_LISTED_IN_LOBBY withObjCType:@encode(nByte)]];

	[mCurrentlyJoinedRoom release];
	mCurrentlyJoinedRoom = [[self createMutableRoom:gameID :roomProps :options.PropsListedInLobby :options.PlayerTtl :options.EmptyRoomTtl] retain];
...
- (bool) opJoinRoom:(NSString*)gameID :(int)playerNumber :(int)cacheSliceIndex
{
	...
	if(!gameID || ![mPeer opCustom:[EGOperationRequest operationRequestWithOperationCode:OperationCode::JOIN_ROOM :op] :true])
		return false;
	[mCurrentlyJoinedRoom release];
	mCurrentlyJoinedRoom = [[self createMutableRoom:gameID :nil :nil :0 :0] retain];
	...
	switch(operationResponse.OperationCode)
	{
		...
		case OperationCode::GET_REGIONS:
		{
			...
			switch(mRegionSelectionMode)
			{
				...
				default:
					[mPeer opAuthenticate:mAppID :mAppVersion :SEND_AUTHENTICATE_ENCRYPTED :mAuthenticationValues :mAutoLobbyStats :mSelectedRegion=[mAvailableRegions objectAtIndex:0]];
					break;
				...
				case EGRegionSelectionMode_BEST:
					if(self.RegionWithBestPing)
						[mPeer opAuthenticate:mAppID :mAppVersion :SEND_AUTHENTICATE_ENCRYPTED :mAuthenticationValues :mAutoLobbyStats :mSelectedRegion=self.RegionWithBestPing];
					else
...
- (void) onPingResponse:(NSString*)address :(unsigned int)result
{
	...
	[mPeer opAuthenticate:mAppID :mAppVersion :SEND_AUTHENTICATE_ENCRYPTED :mAuthenticationValues :mAutoLobbyStats :mSelectedRegion=mRegionWithBestPing=mAvailableRegions[indexOfRegionWithBestPing]];
}
